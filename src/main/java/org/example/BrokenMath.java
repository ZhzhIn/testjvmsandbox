...
@Information(id = "broken-math-repair")
public class BrokenMath implements Module {
    ...
    @Command("repairMathException")
    public void repairMathException() {

        new EventWatchBuilder(moduleEventWatcher)
                .onClass("com.patech.MathMethod")
                .onBehavior("testException")
                .onWatch(new AdviceListener() {
                    /**
    ...
    @Command("repairMathExceptionNum")
    public void repairMathExceptionNum() {

        new EventWatchBuilder(moduleEventWatcher)
                .onClass("com.patech.MathMethod")
                .onBehavior("testExceptionNum")
                .onWatch(new AdviceListener() {
                    /**
                     * 拦截{@code com.patech.TestJvmSandbox#testException()}方法，当这个方法抛出异常时将会被
                     * AdviceListener#afterThrowing()所拦截
                    ...
                    protected void afterThrowing(Advice advice) throws Throwable {
                        ...
                        // 在此，你可以通过ProcessController来改变原有方法的执行流程
                        // 这里的代码意义是：改变原方法抛出异常的行为，变更为立即返回；void返回值用null表示
                        ProcessController.returnImmediately(0);
                    }
                });
    }
}